...
import { EPasswordType, PASSWORD_ICONS, type TPasswordTypes } from 'src/constants/password-visibility.constants';

export function useVisibilityPassword() {
  const passwordVisibility = ref<TPasswordTypes>(EPasswordType.PASSWORD);

  const getPasswordIcon = computed<string>(() => PASSWORD_ICONS[passwordVisibility.value]);
